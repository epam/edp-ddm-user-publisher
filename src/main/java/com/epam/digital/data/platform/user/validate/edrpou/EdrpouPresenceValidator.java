...
public class EdrpouPresenceValidator extends Validator {

  @Override
  public ValidationResult validate(int userSequenceNumber, CsvUser user, ValidationResult results) {
    if (StringUtils.isNullOrEmpty(user.getEdrpou())) {
      return results.add(userSequenceNumber, "User does not contain EDRPOU");
    }
