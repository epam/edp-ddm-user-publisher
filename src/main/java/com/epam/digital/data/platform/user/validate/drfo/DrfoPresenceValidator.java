...
public class DrfoPresenceValidator extends Validator {

  @Override
  public ValidationResult validate(int userSequenceNumber, CsvUser user, ValidationResult results) {
    if (StringUtils.isNullOrEmpty(user.getDrfo())) {
      return results.add(userSequenceNumber, "User does not contain DRFO");
    }
