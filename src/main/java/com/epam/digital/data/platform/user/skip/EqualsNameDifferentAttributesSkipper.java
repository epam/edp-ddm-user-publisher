...
public class EqualsNameDifferentAttributesSkipper extends Skipper {

  private final ExistingUsersProvider existingUsersProvider;

  public EqualsNameDifferentAttributesSkipper(ExistingUsersProvider existingUsersProvider) {
  ...
  public SkippingResult check(List<User> users, SkippingResult results) {
    ...
    var usersWithEqNameDiffAttributes = getUsersWithEqNameDiffAttributes(existingUsers, users);
    for (var entry : usersWithEqNameDiffAttributes.entrySet()) {
  ...
  private String createMessage(Entry<Integer, User> entry) {
    return String.format("The user with name '%s' will be skipped because there is already a user "
            + "with the same name but different attributes (drfo, edrpou, fullName)",
