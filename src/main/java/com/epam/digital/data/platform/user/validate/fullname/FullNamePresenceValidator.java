...
public class FullNamePresenceValidator extends Validator {

  @Override
  public ValidationResult validate(int userSequenceNumber, CsvUser user, ValidationResult results) {
    if (StringUtils.isNullOrEmpty(user.getFullName())) {
      return results.add(userSequenceNumber, "User does not contain FullName");
    }
