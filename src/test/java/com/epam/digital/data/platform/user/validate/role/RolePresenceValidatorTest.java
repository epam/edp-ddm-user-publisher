...
class RolePresenceValidatorTest {

  private RolePresenceValidator validator;

  @BeforeEach
  void beforeEach() {
    validator = new RolePresenceValidator();
  }
  ...
  @Test
  void writeErrorWhenRolesIsNull() {
    List<String> roles = new ArrayList<>();
    roles.add(null);

    var result = validator.validate(1, user().realmRoles(roles).build(), new ValidationResult());

    assertThat(result).hasSize(1);
    assertThat(result.get(1).get(0)).isEqualTo("Role cannot be null or empty string");
  }
